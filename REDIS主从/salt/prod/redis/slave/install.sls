...
unzip-redis:
  archive.extracted:
    - name: /usr/src
    - source: salt://redis/slave/files/redis-6.2.6.tar.gz
    - if_missing: /usr/src/redis-6.2.6

redis-install.sh: 
  cmd.script: 
    - name: salt://redis/slave/files/install.sh.j2
    - template: jinja
    - require: 
      - archive: unzip-redis
...
{{ pillar['redis-installdir'] }}/conf/redis.conf: 
  file.managed: 
    - source: salt://redis/slave/files/redis_slave.conf.j2
    - template: jinja

/usr/lib/systemd/system/redis_server.service:
  file.managed: 
    - source: salt://redis/slave/files/redis_server.service.j2 
    - template: jinja
